...
制作软件（此处使用WinPE，注意2.2之前版本的WinPE内核识别不了搭载十一代cpu机器的硬盘，所以注意要用2.2及以上的新版本）
...
现在主流选择的镜像都是Windows10 64位，镜像可以去

[MSDN, 我告诉你 - 做一个安静的工具站 (itellyou.cn)](https://msdn.itellyou.cn/)

[NEXT, ITELLYOU](https://next.itellyou.cn/)

进行下载。在找到需要的镜像之后，复制镜像的ed2k链接，使用迅雷或者其他的下载器下载镜像

镜像下载好之后，找到镜像的下载位置，将其复制到制作好的pe盘中。至此，准备工作完成
...
* 此处需要将电脑的C盘格式化，务必在**操作之前告知机主**，并帮助同学进行**重要资料的备份和转移**
* DiskGenius中显示的C盘**不一定**是机器的系统盘，此时可打开此电脑让机主对C盘进行辨认，以防因为盘符不同造成机主的资料丢失
...
机器重启后，会进入重装系统的下一个阶段，此时等待，直至小娜的提示音出现。（***如果在公共场所，务必提前关闭声音，避免社死（doge）***）
